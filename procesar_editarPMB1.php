...
if ($currentItemStock < $newStock) {
    ...
    echo "<script>
            alert('Error: El stock actual del Item es $currentItemStock. No puedes prestar más de lo disponible.');
...
if (mysqli_query($conn, $sql)) {
    ...
    header("Location: PrestamoMB1.php?status=success");
} else {
    // Si ocurre un error, redirigir con un mensaje de error
    $error = mysqli_error($conn);
    header("Location: PrestamoMB1.php?status=error&error=" . urlencode($error));
}
