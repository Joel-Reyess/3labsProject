...
if (isset($_POST['btnEliminar'])) {
    ...
    if ($row_check['total'] > 0) {
        ...
        header("Location: MaterialesNave.php?error=No se puede eliminar el registro porque tiene un préstamo asociado.");
        exit();
    } else {
        // Si no tiene préstamos, eliminar el registro
        $sql = "DELETE FROM materiales where ItemID= '$itemid'";
        if (mysqli_query($conn, $sql)) {
            header("Location: MaterialesNave.php?status=deleted");
            exit();
        } else {
            header("Location: MaterialesNave.php?error=No se pudo eliminar el registro.");
            exit();
        }
    }
}
