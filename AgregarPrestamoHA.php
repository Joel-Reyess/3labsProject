...
try {
    ...
    if ($stockActual <= 0 || $stockActual < $Stock) {
        // Redirigir a PrestamoHA.php con un mensaje de error en la URL
        header("Location: PrestamoHA.php?status=error&message=No%20hay%20suficiente%20stock%20disponible");
        exit();
    }
    ...
    header("Location: PrestamoHA.php?status=success");
    exit();
