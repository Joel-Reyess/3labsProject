...
if (isset($_POST['btnEliminar'])) {
    ...
    if ($row_check['total'] > 0) {
        ...
        header("Location: Alumnos.php?error=No se puede eliminar el estudiante porque tiene un préstamo asociado.");
        exit();
    } else {
        // Si no tiene préstamos, eliminar el registro
        $sql = "DELETE FROM estudiantes WHERE Matricula = '$matricula'";
        if (mysqli_query($conn, $sql)) {
            header("Location: Alumnos.php?status=deleted");
            exit();
        } else {
            header("Location: Alumnos.php?error=No se pudo eliminar el registro.");
            exit();
        }
    }
}
