...
if (isset($_POST['btnEliminar'])) {
    ...
    if ($row_check['total'] > 0) {
        ...
        header("Location: IndexA1.php?error=No se puede eliminar el estudiante porque tiene un préstamo asociado.");
        exit();
    } else {
        // Si no tiene préstamos, eliminar el registro
        $sql = "DELETE FROM herramientas where ItemID= '$ItemID'";
        if (mysqli_query($conn, $sql)) {
            header("Location: IndexA1.php?status=deleted");
            exit();
        } else {
            header("Location: IndexA1.php?error=No se pudo eliminar el registro.");
            exit();
        }
    }
}
